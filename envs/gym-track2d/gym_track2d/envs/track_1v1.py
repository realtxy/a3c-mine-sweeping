...
class Track1v1Env(gym.Env):
    ...
    def step(self, action):
        ...
        action = list(action)
        # move agents
        x1 = np.array(self.state[0])[0]
        x2 = np.array(self.state[1])[0]
        y1 = np.array(self.state[0])[1]
        y2 = np.array(self.state[1])[1]
        d1 = abs(x1 - x2)
        d2 = abs(y1 - y2)
        if d1 > self.pob_size or d2 > self.pob_size:
            action[1]='4'
            if y1+self.fx<=80 and y1+self.fx>=1:
                if self.fx==1:
                   action[0]='3'
                elif self.fx==-1:
                    action[0] = '2'
            elif y1+self.fx>80 or y1+self.fx<1:
                action[0]='1'
                self.fx = -self.fx
        elif d1 <= self.pob_size and d2 <= self.pob_size:
            for i in range(self.num_agents - 1):
                if 'Ram' in self.target_mode:
        ...
        for i in range(self.num_agents):
            self.state[i], self.C_collision[i] = self._next_state(i, self.state[i], int(action[i]),
                                                                  self.action_type)
            print(i,"state is:",self.state[i])
    ...
    def _next_state(self, player, state, action, action_type='VonNeumann'):
        ...
        if action_type == 'VonNeumann':
            transitions = {0: [-1, 0], 1: [+1, 0], 2: [0, -1], 3: [0, +1], 4: [0, 0]}
        ...
        if self.maze[new_state[0]][new_state[1]] == 1:  # Hit wall, stay there
            if new_state[0]>=1 and new_state[0]<=80 and new_state[1]>=1 and new_state[1]<=80:
                if player==0:
